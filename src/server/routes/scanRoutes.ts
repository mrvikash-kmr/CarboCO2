...
const analyzeUrl = async (targetUrl: string) => {
  ...
  let breakdown = { html: 0, css: 0, js: 0, image: 0, font: 0, other: 0 };
...
router.post('/analyze', async (req, res) => {
  try {
    ...
    res.json({ id: Date.now().toString(), ...result });
  } catch (error: any) {
...
router.post('/compare', async (req, res) => {
  try {
    ...
    if (!url1 || !url2) return res.status(400).json({ error: "Both URLs are required" });

    const [res1, res2] = await Promise.all([analyzeUrl(url1), analyzeUrl(url2)]);

    res.json({ site1: res1, site2: res2 });
  } catch (error: any) {
